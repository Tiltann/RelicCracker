...
import type { DevScanEvent, FileScanResult } from "../types";
import { REWARD_THRESHOLD, TMPL_X, TMPL_Y, TMPL_W, TMPL_H, REF_W, REF_H, DEFAULT_OCR_Y_MIN_PCT } from "../devConstants";
...
function ScanRow({ e, expanded, onToggle }: {
  e: DevScanEvent;
  expanded: boolean;
  onToggle: () => void;
}) {
  const scoreColor =
    e.sad_score === null ? "text-wf-muted" :
    e.template_matched ? "text-wf-success" : "text-wf-danger";
  ...
  return (
    <div className="border-b border-wf-border/40 last:border-0">
      ...
      >
        ...
        <span className={`w-[60px] shrink-0 ${e.template_matched ? "text-wf-success" : "text-wf-muted"}`}>
          {e.template_matched ? "match" : "miss"}
        </span>
        <span className="text-wf-muted w-[55px] shrink-0">{e.duration_ms}ms</span>
        ...
        {e.ocr_lines.length > 0 && e.items_found.length === 0 && (
          <span className="text-wf-muted text-[10px]">{e.ocr_lines.length} lines, no match</span>
        )}
        {(expanded || e.ocr_lines.length > 0) && (
          <span className="ml-auto text-wf-muted text-[10px]">{expanded ? "hide" : "show"}</span>
        )}
      </button>
          ...
          {e.items_found.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-1">
              {e.items_found.map((it, i) => (
                <span key={i} className="text-[10px] bg-wf-accent/15 text-wf-accent px-1.5 py-0.5 rounded">{it}</span>
              ))}
            </div>
          )}
          {e.ocr_lines.map((line, i) => (
            <div key={i} className="font-mono text-[10px] text-wf-muted leading-tight pl-2">{line}</div>
          ))}
        </div>
      )}
    </div>
...
function ScanDiagram({ ocrYMinPct }: { ocrYMinPct: number }) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      >
        {/* OCR active band */}
        <rect x={0} y={oy} width={VW} height={yMaxLine - oy}
          fill="rgba(82,130,224,0.12)" stroke="none" />
        <rect x={0} y={oy} width={VW} height={yMaxLine - oy}
          fill="none" stroke="#5284e0" strokeWidth="0.8" />
        {/* Template SAD region */}
        <rect x={tx} y={ty} width={tw} height={Math.max(th, 2)}
          fill="rgba(196,154,60,0.2)" stroke="#c49a3c" strokeWidth="0.8" />
        {/* Y-min line */}
        <line x1={0} y1={oy} x2={VW} y2={oy} stroke="#5284e0" strokeWidth="0.6" strokeDasharray="3,2" />
        <text x={2} y={oy - 1.5} fontSize="4" fill="#5284e0" opacity="0.7">{ocrYMinPct.toFixed(1)}%</text>
        {/* Y-max line */}
        <line x1={0} y1={yMaxLine} x2={VW} y2={yMaxLine} stroke="#5284e0" strokeWidth="0.6" strokeDasharray="3,2" opacity="0.5" />
        <text x={2} y={yMaxLine + 5} fontSize="4" fill="#5284e0" opacity="0.5">85%</text>
        <text x={tx + tw / 2} y={ty + 8} textAnchor="middle" fontSize="4" fill="#c49a3c">SAD</text>
      </svg>
      <div className="flex flex-col gap-0.5 text-[10px]">
        ...
        <div className="text-wf-muted/60 mt-0.5">SAD threshold: {REWARD_THRESHOLD.toLocaleString()}</div>
      </div>
    </div>
  );
}
...
function PngScanner({ ocrYMin }: { ocrYMin: number }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <span className="text-wf-muted uppercase text-[9px] tracking-widest font-semibold">Scan PNG</span>
      ...
      {fileName && !loading && (
        <div className="text-[9px] text-wf-muted/60 truncate" title={fileName}>{fileName}</div>
      )}
      ...
      {result && (
        <div className="flex flex-col gap-1.5">
          ...
          {result.db_item_count === 0 ? (
            ...
            <div className="text-[9px] text-wf-muted/50">DB: {result.db_item_count.toLocaleString()} items</div>
          )}
            ...
            {result.matched_items.length === 0 ? (
              ...
              <div className="flex flex-col gap-0.5">
                {result.matched_items.map((it, i) => (
                  <div key={i} className="text-[10px] text-wf-accent font-medium">{it}</div>
                ))}
              </div>
            )}
          </div>
          ...
          <div>
            ...
            {result.raw_lines.length === 0 ? (
              <div className="text-[10px] text-wf-danger">No text read from image</div>
            ) : (
              <div className="flex flex-col gap-0.5 pl-1 border-l border-wf-border/40 max-h-[90px] overflow-y-auto">
                {result.raw_lines.map((line, i) => (
                  <div key={i} className="font-mono text-[9px] text-wf-muted/80 leading-tight">{line}</div>
                ))}
              </div>
            )}
          </div>
  ...
  return (
    <div className="shrink-0 border-t border-wf-accent/40 bg-wf-surface text-[11px]">
      ...
      <div className="flex items-center gap-3 px-3 py-1.5 border-b border-wf-border/60">
        <span className="text-wf-accent font-semibold tracking-wide uppercase text-[10px]">Dev Mode</span>
        ...
        {lastMatch && (
          <span className="text-wf-muted">
            ...
            {lastMatch.items_found.length > 0 && (
              <> - <span className="text-wf-success">{lastMatch.items_found.join(", ")}</span></>
            )}
          </span>
        )}
        <div className="ml-auto flex items-center gap-2">
          <button onClick={() => setEntries([])} className="text-wf-muted hover:text-wf-text cursor-pointer transition-colors">clear</button>
          <button onClick={() => setCollapsed(c => !c)} className="text-wf-muted hover:text-wf-text cursor-pointer transition-colors px-1">
            {collapsed ? "show" : "hide"}
          </button>
        </div>
      </div>
      ...
      {!collapsed && (
        <div className="flex" style={{ height: 300 }}>
          ...
          <div className="w-[220px] shrink-0 p-3 border-r border-wf-border/60 flex flex-col gap-3 overflow-y-auto">
            <div className="flex flex-col gap-2">
              <span className="text-wf-muted uppercase text-[9px] tracking-widest font-semibold">Scan Regions</span>
              <ScanDiagram ocrYMinPct={ocrYMin} />
              <div className="flex flex-col gap-1">
                <div className="flex items-center justify-between text-[10px]">
                  <span className="text-wf-muted">OCR Y-min</span>
                  <div className="flex items-center gap-1">
                    <span className="text-wf-text font-mono">{ocrYMin.toFixed(2)}%</span>
                    <button
                      onClick={() => {
                        setOcrYMin(DEFAULT_OCR_Y_MIN_PCT);
                        invoke("set_ocr_threshold", { pct: DEFAULT_OCR_Y_MIN_PCT });
                      }}
                ...
                <input
                  type="range" min="0" max="80" step="0.5" value={ocrYMin}
                  onChange={e => {
