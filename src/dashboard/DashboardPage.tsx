    ...
    <div className="max-w-[920px] flex flex-col gap-5 animate-fade-up">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div
            className="flex items-center gap-2 px-3 py-1.5 rounded-[5px] text-[11.5px]"
            style={{
              background: "#0e1016",
      ...
      {history.length > 0 && (
        ...
        >
          {/* Table header */}
          <div
            className="grid grid-cols-[150px_68px_1fr_28px] px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.09em]"
            style={{
              background: "#0e1016",
              color: "#3a4050",
          ...
          {history.map((row, ri) => {
            const rewards: RewardResult[] = (() => {
              try {
                return JSON.parse(row.rewards_json);
              } catch {
                return [];
              }
            })();
            ...
            return (
              <div
                ...
                style={{
                  borderBottom:
                    ri < history.length - 1 ? "1px solid #14171e" : undefined,
                  animation: `fade-up 200ms ease ${ri * 30}ms both`,
                }}
                onMouseEnter={(e) =>
                  (e.currentTarget.style.background = "#0e1016")
                }
                onMouseLeave={(e) =>
              ...
              >
                {/* Time */}
                <span
                  className="text-[11px] pt-[3px] whitespace-nowrap tabular-nums"
                  style={{ color: "#3a4050" }}
                >
  ...
  const styles: Record<string, { color: string; bg: string; border: string }> =
    {
      log: {
        color: "#52c27a",
        bg: "rgba(82,194,122,0.1)",
        border: "rgba(82,194,122,0.2)",
      },
      ocr: {
        color: "#5284e0",
        bg: "rgba(82,132,224,0.1)",
        border: "rgba(82,132,224,0.2)",
      },
      screen: {
        color: "#5284e0",
        bg: "rgba(82,132,224,0.1)",
      ...
      manual: {
        color: "#9a7a50",
        bg: "rgba(154,122,80,0.1)",
        border: "rgba(154,122,80,0.2)",
      },
      test: {
        color: "#7a8090",
        bg: "rgba(122,128,144,0.1)",
...
function RewardChip({ reward }: { reward: RewardResult }) {
  return (
    ...
    >
      ...
      {reward.median_plat != null && (
        <div className="flex items-center gap-[3px]">
          <img
            src={platIcon}
            alt=""
            className="w-[9px] h-[9px] shrink-0 opacity-70"
          />
          <span
      ...
      {reward.ducats > 0 && (
        <div className="flex items-center gap-[2px]">
          ...
          <span
            ...
            style={{ color: reward.is_best ? "#d4953a" : "#5a6070" }}
          >
            {reward.ducats}
          </span>
        </div>
      ...
      {reward.vaulted && (
        <span
          className="text-[8px] font-bold uppercase px-[3px] py-[1px] rounded-[2px]"
          style={{
            color: "#e07070",
...
function EmptyState() {
  return (
    <div
      className="flex flex-col items-center justify-center py-14 rounded-[8px] gap-4 animate-fade-in"
      style={{ background: "#0e1016", border: "1px solid #1c1f27" }}
    >
      <div className="flex flex-col items-center gap-1.5">
        <span className="text-[13px]" style={{ color: "#3a4050" }}>
          No sessions yet
        </span>
        <span
          className="text-[11px] text-center max-w-[260px]"
          style={{ color: "#2a3040" }}
        >
...
function HotkeyHint({ label, hotkey }: { label: string; hotkey: string }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[10px]" style={{ color: "#2a3040" }}>
        {label}
      </span>
    </div>
  );
